...
файла описан в точке зрения Интерфейсы.
...
- Число в ячейке мира задаёт "hole", который можно заполнить с помощью нуля,
  одного или нескольких hole filler.
- Hole filler `Player` размещает в указанном `hole` персонажа.
...
- Hole filler `Item` требует в качестве `HoleFillerData` значение `Sword` или `GoldenApple` и
  размещает соответствующий предмет.
