...
- **AbstractMob** — абстрактный класс враждебного существа, наследуется от **BehaviourMovable** и
  реализует **Trigger** для боя с игроком.
...
Класс **AbstractMob**, помимо расширения **BehaviourMovable** и реализации **Trigger**,
предоставляет свойства с характеристиками враждебного существа (текущее здоровье, максимальное
здоровье, сила атаки, броня), а также число единиц опыта, которые даются игроку за убийство этого
существа. При бое с игроком происходит вычисление урона, наносимого игроку и враждебному существу.
...
**TemporalBehaviourModel** в совокупности с **ConfusedBehaviourModel**, заставляя враждебное
существо перемещаться случайно в течение нескольких ходов.

Интерфейс **BehaviourModel** представляет некую модель поведения для **Movable** объекта.
Единственный предоставляемый метод — *onMove*, предназначен для вызова во время хода игрока и
совершения хода объектом.
...
- **ConfusedBehaviourModel** — ходит в случайном направлении,
...
<!-- здесь потом будет про КА поведений мобов -->
